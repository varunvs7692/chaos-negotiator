...
class EnforcementSimulator:
    ...
    async def simulate_deployment(
        self, contract: DeploymentContract, failure_scenario: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        for g in contract.guardrails:
            if hasattr(g, "metric_name"):
                ...
                metric_name = (
                    f"max_{g.guardrail_type.value}_percent"
                    if "rate" in g.guardrail_type.value
                    else f"max_{g.guardrail_type.value}_ms"
                )
                guardrails_dict[metric_name] = type(
                    "obj",
                    (object,),
                    {
        ...
        for phase_idx, traffic_pct in enumerate(traffic_phases):
            ...
            for check in range(3):  # 3 checks per phase
                ...
                # Check guardrails
                violation = self._check_guardrails(metrics, max_error_rate, max_p95_latency)

                if violation:
                    violations.append(violation)
                    logger.warning(f"⚠️  GUARDRAIL VIOLATION: {violation['metric']}")
                    logger.warning(
                        f"   Actual: {violation['actual']}, Threshold: {violation['threshold']}"
                    )
    ...
    def _check_guardrails(
        self, metrics: SimulatedMetrics, max_error_rate: float, max_p95_latency: float
    ...
    async def _execute_rollback(self, contract: DeploymentContract) -> Dict[str, Any]:
        ...
        for idx, step in enumerate(steps):
            logger.info(
                f"  Step {idx + 1}: {step.description if hasattr(step, 'description') else step}"
            )
...
async def run_enforcement_demo(
    contract: DeploymentContract, scenario: str = "success"
) -> Dict[str, Any]:
    """
    ...
    failure_map = {"success": None, "error_spike": "error_spike", "latency_spike": "latency_spike"}
